...
- Fullscreen mode: an SVG toggle button in the bottom-right corner of canvas; auto-hides after 2 s of inactivity in fullscreen; reappears on mouse/pointer/key activity. `app.js` uses a `ResizeObserver` on `#art-container` to dispatch `window.resize` so art modules resize correctly on fullscreen enter/exit.
